...
class ZhCrawler(FlightCrawler):
        ...
        try:
            ...
            self.context = await self.browser.new_context(
                ...
                extra_http_headers={
                    ...
                    "Referer": "https://www.shenzhenair.com/",
                },
            )
            ...
            raise BrowserCrashError(f"zh browser init failed: {exc}") from exc
    ...
    async def search_flights(self, route: Route) -> List[Dict[str, Any]]:
        ...
        for index, flight_date in enumerate(sorted(route.dates.absolute_dates)):
            ...
            try:
                ...
                logger.error(
                    "[zh] search failed for %s -> %s on %s: %s",
    ...
    async def _search_single_date(
        ...
        """搜索单个日期的航班"""
        if not self.context:
            raise CrawlerError("zh browser context is not initialized")
    ...
    def _build_search_url(self, route: Route, flight_date: date) -> str:
        """构建搜索 URL"""
        from_code = get_zh_city_code(route.from_city)
        to_code = get_zh_city_code(route.to_city)
        # 深航搜索 URL 格式（需要根据实际页面调整）
        date_str = flight_date.strftime("%Y-%m-%d")
        return f"https://www.shenzhenair.com/szair/booking/bookOneWay?from={from_code}&to={to_code}&date={date_str}"
    ...
    async def _warm_session(self) -> None:
        ...
        page = await self.context.new_page()
        try:
            await page.goto(ZH_HOMEPAGE_URL, wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(random.randint(3000, 5000))
            self._session_warmed = True
        except Exception as exc:
            logger.info("[zh] warm-up page skipped: %s", exc)
        finally:
            await page.close()
    ...
    async def _parse_flights(
        ...
        for selector in selectors:
            try:
                ...
                if items:
                    logger.info("[zh] found %s flight items via %s", len(items), selector)
                    break
            except Exception:
                continue
    ...
    async def _parse_single_flight(
        ...
        try:
            ...
            # 提取航班号（ZH开头）
            flight_match = re.search(r"\b(ZH\d{3,4})\b", text, re.IGNORECASE)
            if not flight_match:
                return None
    ...
    async def _save_debug_snapshot(
        ...
        """保存调试快照"""
        safe_name = f"zh_{route.from_city}_{route.to_city}_{flight_date.isoformat()}_{suffix}"
