...
def get_ho_city_code(city_name: str) -> str:
    """获取吉祥航空城市代码"""
    return HO_CITY_CODE_MAP.get(city_name, city_name)


class HoCrawler(FlightCrawler):
    """吉祥航空爬虫"""
    ...
    async def init(self) -> None:
        ...
        try:
            ...
            raise BrowserCrashError(f"ho browser init failed: {exc}") from exc
    ...
    async def search_flights(self, route: Route) -> List[Dict[str, Any]]:
        ...
        for index, flight_date in enumerate(sorted(route.dates.absolute_dates)):
            ...
            try:
                ...
                logger.error(
                    "[ho] search failed for %s -> %s on %s: %s",
    ...
    async def _search_single_date(
        ...
        """搜索单个日期的航班"""
        if not self.context:
            raise CrawlerError("ho browser context is not initialized")
    ...
    def _build_search_url(self, route: Route, flight_date: date) -> str:
        """构建搜索 URL"""
        from_code = get_ho_city_code(route.from_city)
        to_code = get_ho_city_code(route.to_city)
        # 吉祥航空搜索 URL 格式（需要根据实际网站调整）
        return f"https://www.juneyaoair.com/booking/{from_code}-{to_code}-{flight_date.strftime('%Y%m%d')}/"
    ...
    async def _warm_session(self) -> None:
        ...
        page = await self.context.new_page()
        try:
            await page.goto(HO_HOMEPAGE_URL, wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(random.randint(3000, 5000))
            self._session_warmed = True
        except Exception as exc:
            logger.info("[ho] warm-up page skipped: %s", exc)
        finally:
            await page.close()
    ...
    async def _parse_flights(
        ...
        for selector in selectors:
            try:
                ...
                if items:
                    logger.info("[ho] found %s flight items via %s", len(items), selector)
                    break
            except Exception:
                continue
    ...
    async def _parse_single_flight(
        ...
        try:
            ...
            # 提取航班号（HO开头）
            flight_match = re.search(r"\b(HO\d{3,4})\b", text, re.IGNORECASE)
            if not flight_match:
                return None
    ...
    async def _save_debug_snapshot(
        ...
        """保存调试快照"""
        safe_name = f"ho_{route.from_city}_{route.to_city}_{flight_date.isoformat()}_{suffix}"
